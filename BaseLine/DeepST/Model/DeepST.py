...
class run():
	...
	def _get_adata(
		self,
		platform, 
		data_path,  # 基础路径，指向切片目录
		data_name,  # 切片名称
		count_file="filtered_feature_bc_matrix.h5",  # 默认文件名
		verbose=False
	):
		"""
		...
		"""
		# 确保平台合法
		assert platform in ['Visium', 'ST', 'MERFISH', 'slideSeq', 'stereoSeq'], "Unsupported platform!"

		# 拼接完整路径
		full_path = os.path.join(data_path, data_name)  # 切片完整目录路径
		...
		if platform in ['Visium', 'ST']:
			if platform == 'Visium':
				...
				adata = read_10X_Visium(
					path=full_path,  # 切片目录路径
					count_file=count_file  # 文件名
				)
				
		elif platform == 'MERFISH':
			# 读取 MERFISH 数据
			adata = read_merfish(full_path)
		elif platform == 'slideSeq':
			# 读取 SlideSeq 数据
			adata = read_SlideSeq(full_path)
		elif platform == 'stereoSeq':
			# 读取 StereoSeq 数据
			adata = read_stereoSeq(full_path)
		else:
			raise ValueError(f"Platform {platform!r} does not support.")

		# 如果 verbose 为 True，则保存原始数据到文件
		if verbose:
			save_data_path = Path(self.save_path) / "Data" / data_name
			save_data_path.mkdir(parents=True, exist_ok=True)
			adata.write(
				filename=save_data_path / f"{data_name}_raw.h5ad",
				compression="gzip"
			)
